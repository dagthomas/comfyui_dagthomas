...
The chain carry hands H3 the previous scene's last frames — the actor's identity, pose, direction and speed continue exactly — but not the previous *place*, so a change of place inside a continuing take has to be written, and left unsaid the writers keep continuing scenes in the room they started in. The Short Film, Crossover and Scenes writers (in *Continuous chain* mode) and the Music Video Writer (for pieces marked `CONTINUES the previous take`) end with a `transition_style` dropdown: **Auto** (default) — the writer picks walk‑through or reveal per scene where the plan moves; **Stay** — same place, always; **Walk‑through** — the actor carries the take through a door, corridor or corner into the new place, the camera following without cutting, the new place's anchors and light named as they come into frame behind and around the moving actor; **Reveal** — the actor holds while the camera moves and a new space is disclosed around them, completed within the first half of the scene. Hard cuts are unaffected, and a scene whose plan stays put stays put.
