...
SamplerCustomAdvanced and Save Clip in **one node**. ComfyUI maps a scene list over each node before moving to the next, so with the two separate nodes all 20 latents sample first and nothing reaches disk until the last one — and a crash at scene 19 loses everything. This node takes the sampler's inputs (`noise`, `guider`, `sampler`, `sigmas`, `latent_image`) plus Save Clip's (`vae`, `audio`, `filename_prefix`, `fps`, `format`) and, per list item, samples → decodes → muxes the audio slice → writes the clip **before the next scene starts sampling**: the output folder fills up while the run is still going, exactly like Chain Render. `output` is the sampled latent, so anything that hung off the sampler (VAE Decode Audio, an upscaler) still does. **`audio_vae`** (optional): with `audio` unplugged and the H3 audio VAE connected, the clip's soundtrack is the audio half of the *sampled* latent, decoded — the sound H3 generated in this pass, nothing muxed on afterwards. That is the setting the **Comfy H3 Sync Sound challenge** requires ("the audio has to come out of the same H3 pass as the video"); the writer's `audio_segments` on `audio` would put the original song on the clip instead. `audio` connected always wins; neither connected = silent clips. Code: `nodes/h3/sample_and_save.py`.
...
The conditioning mix a mix engineer would make. H3 lip‑syncs to the audio in its latent and moves the picture to the rhythm it hears there, and every extreme loses something: the full song buries the consonants (mouths drift), a bare vocal stem has no beat in it (nothing to cut on), a synthetic thump track is not the song. This node puts the **voice on top at full level** (`voice`, AudioSeparation's Vocals) and the **real music under it** at `music_db` (default **‑9 dB**; `music_1..3` are summed — the whole song, or Drums + Bass + Other for the instrumental without a second copy of the voice), and with `duck_db` (default **‑6 dB**) a sidechain dips the music a little more only while the voice is actually sounding (5 ms in / 150 ms out), so words stay readable and the drums come back up between phrases. Peak‑normalised. Wire `audio` into Chain Render's **`conditioning_audio`** (or the masked‑audio node's `master_audio`); with the song frozen in the latent the original song still goes to the output. **Reference mode** (Masked Song Latent at `audio_denoise` > 0 with the output audio decoded from the sampled latent — the Sync Sound challenge setting) is different: H3 regenerates the sound *from* this mix, so whatever is ducked here is thin in the output. Two controls keep the music in that case, both appended after the old widgets so saved workflows keep their positions: **`duck_band`** — *full band* is the old fader; *vocal band only* ducks just 300 Hz–4 kHz (the range that masks consonants; half‑octave raised‑cosine edges, linear phase, inside + outside = the bed exactly), so kick, bass and hats stay at full level straight through the vocal — and **`music_1_db..3_db`**, a trim per stem before the sum, so Drums can sit at 0 while Other (pads, keys, guitars in the vocal range) sits at −3 to −6. Reference‑mode starting point: `music_db` −3, `duck_db` −4, vocal band, `music_3_db` −3. Code: `nodes/h3/voice_mix.py`; tests: `tests/test_voice_mix.py`.
...
| [`h3_music_video_sync_sound_challenge.json`](examples/h3/h3_music_video_sync_sound_challenge.json) | **Sync Sound challenge edition** of the Ollama masked‑song + stems video — the audio in the output is **H3's own**, as the challenge rules require ("audio from the same H3 pass as the video"; a reference clip to steer is allowed, a separately‑made track muxed on afterwards is not). Three changes: H3 Masked Song Latent at `audio_denoise` **0.4** (sung) / `gap_denoise` **0.9** (gaps) so the song is a masked *reference* H3 re‑synthesises rather than a frozen pass‑through; Sample + Save with `audio` **unplugged** and `audio_vae` connected, so every clip carries the audio decoded from its sampled latent; Stitch Clips with `song` **unplugged**; Voice Over Music on the reference‑mode mix (bed −3 dB, vocal‑band duck −4 dB, Other −3 dB) so the bed H3 regenerates from is not thin. The notes spell out the rules, the knobs, the purist `ref_audio_0` variant and what to disclose in the submission. |
